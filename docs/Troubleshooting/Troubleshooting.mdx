...
2. Add the token to MOS via **Settings → Network → Token**
...
:::tip Explanation: MOS has a built-in **60-second rollback mechanism**. After applying network changes, you must confirm the new configuration within 60 seconds by navigating to **Settings → Network → Network Interfaces** and clicking **Accept**.
...
:::tip Solution:
Navigate to **Settings → Virtualization → Docker service** and make sure Docker is enabled and started. You can also restart the Docker service from there.
:::
...
:::tip Solution:
Navigate to **Settings → Virtualization → LXC service** and make sure LXC is enabled and started.
:::
...
- Check that the VM service is running via **Settings → Virtualization → VM service**
:::
